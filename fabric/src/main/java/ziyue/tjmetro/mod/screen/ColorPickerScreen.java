...
public class ColorPickerScreen extends ScreenExtension implements IGui
{
    ...
    private final Identifier paletteIdentifier = new Identifier("tjmetro", "color_picker/" + UUID.randomUUID());
    private NativeImageBackedTexture paletteTexture;
    private float paletteHue = Float.NaN;
    ...
    public void render(GraphicsHolder graphicsHolder, int mouseX, int mouseY, float delta) {
        ...
        updatePalette();
        final float paletteEnd = (float) PALETTE_SIZE / (PALETTE_SIZE + 1);
        guiDrawing.beginDrawingTexture(paletteIdentifier);
        guiDrawing.drawTexture(SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE + mainWidth, SQUARE_SIZE + mainHeight, 0, 0, paletteEnd, 1);
        guiDrawing.drawTexture(SQUARE_SIZE * 2 + mainWidth, SQUARE_SIZE, SQUARE_SIZE * 3 + mainWidth, SQUARE_SIZE + mainHeight, paletteEnd, 0, 1, 1);
    ...
    private void updatePalette() {
        if (paletteTexture != null && paletteHue == hue) {
            return;
        }
        final NativeImage image = paletteTexture == null ? new NativeImage(NativeImageFormat.getAbgrMapped(), PALETTE_SIZE + 1, PALETTE_SIZE, false) : paletteTexture.getImage();
        for (int row = 0; row < PALETTE_SIZE; row++) {
            for (int column = 0; column < PALETTE_SIZE; column++) {
                image.setPixelColor(column, row, Color.HSBtoRGB(hue, (float) column / (PALETTE_SIZE - 1), 1 - (float) row / (PALETTE_SIZE - 1)));
            }
            image.setPixelColor(PALETTE_SIZE, row, Color.HSBtoRGB((float) row / (PALETTE_SIZE - 1), 1, 1));
        }
        if (paletteTexture == null) {
            paletteTexture = new NativeImageBackedTexture(image);
            MinecraftClient.getInstance().getTextureManager().registerTexture(paletteIdentifier, new AbstractTexture(paletteTexture.data));
        } else {
    ...
    public void removed2() {
        if (paletteTexture != null) {
            MinecraftClient.getInstance().getTextureManager().destroyTexture(paletteIdentifier);
            paletteTexture = null;
        }
        super.removed2();
    }
    ...
    protected int getMainWidth() {
        return Math.max(2, width - SQUARE_SIZE * 5 - RIGHT_WIDTH);
    }

    protected int getMainHeight() {
        return Math.max(2, height - SQUARE_SIZE * 2);
    }
